...
def load_data(use_fixture, user_id):
    if use_fixture:
    ...
    return fetch_transactions(user_id=user_id)


def run(user_id, use_fixture=False, output_dir=None):
    raw = load_data(use_fixture, user_id)
    frame = clean_transactions(raw)

    monthly = monthly_spend(frame, user_id)
    trend = moving_average(monthly)
    breakdown = category_breakdown(frame, user_id)
...
def main():
    ...
    run(args.user_id, use_fixture=args.fixture, output_dir=args.output_dir)
